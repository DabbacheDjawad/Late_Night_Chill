...
export async function GET(req: Request) {
  try {
    ...
    const animewishlists = user.wishlistAnimes.map((a) => ({
      type: "anime" as const,
      id: a.id,
      malId: a.malId,
      title:
    ...
    const moviewishlists = user.wishlistMovies.map((m) => ({
      type: "movie" as const,
      id: m.id,
      tmdbId: m.tmdbId,
      title: m.title,
    ...
    const tvwishlists = user.wishlistTvShows.map((t) => ({
      type: "tv" as const,
      id: t.id,
      tmdbId: t.tmdbId,
      title: t.name,
  ...
  try {
    ...
    const config = (() => {
      switch (mediaType) {
        case "anime":
          return {
            relationField: "wishlistAnimes" as const,
        ...
        case "movie":
          return {
            relationField: "wishlistMovies" as const,
        ...
        case "tv":
          return {
            relationField: "wishlistTvShows" as const,
    ...
    // toggle wishlist
    const existingWishlist = await prisma.user.findFirst({
      where: {
        id: user.id,
    ...
    if (existingWishlist) {
      ...
      return NextResponse.json({ message: "Removed from wishlists" });
    } else {
      await prisma.user.update({
        where: { id: user.id },
        data: { [config.relationField]: { connect: { id: media.id } } },
      });
      return NextResponse.json({ message: "Added to wishlists" });
    }
  } catch (error: any) {
...
export async function DELETE(req: Request) {
  ...
  switch (type) {
    case "movie":
      updateQuery = {
        wishlistMovies: { disconnect: { id: mediaId } },
      };
      break;
    case "tv":
      updateQuery = {
        wishlistTvShows: { disconnect: { id: mediaId } },
      };
      break;
    case "anime":
      updateQuery = {
        wishlistAnimes: { disconnect: { id: mediaId } },
      };
      break;
    default:
